...
// AppSpaceWarp : Enable space warp by default
// You can disable space warp by holding controller.
// Holding trigger : normal 72 fps
// Releasing trigger: SpaceWarp 36 fps mode
static bool EnableSpaceWarp = true;
static bool ShowQuadLayer = true;
static bool QuadLayerFollowing = true;
static bool EnableDeterministicAnimation = false;

// Store camera position and rotation
static XrVector3f cameraPos = {0.0f, 0.0f, 0.0f};
static float rotationY = 0.0f;

// Frame index for deterministic behavior
...
static bool GetSystemPropertyInt(const char* keyName, int* val) {
    char value[128];
...
static const int CUBE_COUNT_DIMENSION = 8;
static const int CUBE_SCALE = 10;
static const int CUBE_NUM_INSTANCES =
    (CUBE_COUNT_DIMENSION * CUBE_COUNT_DIMENSION * CUBE_COUNT_DIMENSION);
enum { ovrMaxCubeNumInstances = CUBE_NUM_INSTANCES };

static const float CUBE_ANIMATION_SPEED = 8.0f;
static const float CUBE_ANIMATION_DISTANCE = 3.0f;
static const float CUBE_ANIMATION_X_OFFSET = 0.7f;
static const float CUBE_ANIMATION_Y_OFFSET = 0.3f;
static const float CUBE_ANIMATION_Z_OFFSET = 0.17f;
static const float FRAME_BASED_CUBE_SPEED_FACTOR = 0.04f; // Factor to roughly match time based animation speed
...
static ovrUniform ProgramUniforms[] = {
    ...
    {PREV_MODEL_MATRIX, MATRIX4X4, "prevModelMatrix"},
    {VIEW_PROJ_MATRIX, MATRIX4X4, "viewProjectionMatrix"},
    {PREV_VIEW_PROJ_MATRIX, MATRIX4X4, "prevViewProjectionMatrix"},
};
...
// Main pass shader : VERTEX_SHADER / FRAGMENT_SHADER are for normal render pass
...
// AppSpaceWarp : MOTION_VECTOR_VERTEX_SHADER / MOTION_VECTOR_FRAGMENT_SHADER are for motion vector
// pass, which will calculate a vertex's previous and current clip space position, convert them
// into NDC in fragment shader, thus the difference of the 2 NDC value are the motion vector, In
// normal video game, usually motion vector pass shader is much simipler compared with the Main pass
// shader, eg. motion vector shader might not need any texture sampling in fragment shader unless it
// can trigger a pixel discard.

static const char MOTION_VECTOR_VERTEX_SHADER[] =
    "#version 300 es\n"
    "in vec3 vertexPosition;\n"
    "in vec4 vertexColor;\n"
    "uniform mat4 viewProjectionMatrix;\n"
...
static const char MOTION_VECTOR_FRAGMENT_SHADER[] =
    "#version 300 es\n"
    "in highp vec4 clipPos;\n"
...
static bool ovrFramebuffer_Create(
    ...
    // AppSpaceWarp : initialize swapchains and frame buffers for motion vector pass
    {
        ...
        for (uint32_t i = 0; i < frameBuffer->TextureSwapChainLength; i++) {
            ...
            GLenum colorTextureTarget = GL_TEXTURE_2D;
            GL(glBindTexture(colorTextureTarget, motionVectorTexture));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
            ...
            GL(glBindTexture(colorTextureTarget, motionVectorDepthTexture));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_MIN_FILTER, GL_NEAREST));
            GL(glTexParameteri(colorTextureTarget, GL_TEXTURE_MAG_FILTER, GL_NEAREST));
            GL(glBindTexture(colorTextureTarget, 0));
            ...
            GL(glFramebufferTexture2D(
                GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, motionVectorTexture, 0));
...
static void ovrFramebuffer_SetCurrent(ovrFramebuffer* frameBuffer, bool isMotionVectorPass) {
    // AppSpaceWarp
...
static void ovrFramebuffer_Resolve(ovrFramebuffer* frameBuffer, bool isMotionVectorPass) {
    if (isMotionVectorPass) {
...
static void ovrFramebuffer_Acquire(ovrFramebuffer* frameBuffer, bool isMotionVectorPass) {
...
static void ovrFramebuffer_Release(ovrFramebuffer* frameBuffer, bool isMotionVectorPass) {
...
typedef struct {
    ...
    XrMatrix4x4f CubeTransforms[ovrMaxCubeNumInstances];
} ovrSceneTransform;
...
typedef struct {
    ...
    ovrGeometry Box;
    ovrSceneTransform SceneTransform;

    ovrSwapChain QuadSwapChain;
    XrSwapchainImageOpenGLESKHR* QuadSwapChainImage;

    // AppSpaceWarp : Shader program for moiton vector pass
    ovrProgram MotionVectorProgram;
} ovrScene;
...
static void ovrScene_Clear(ovrScene* scene) {
    ...
    // AppSpaceWarp
    ovrProgram_Clear(&scene->MotionVectorProgram);
    ovrGeometry_Clear(&scene->GroundPlane);
    ovrGeometry_Clear(&scene->Box);
    for (int i = 0; i < 4; i++) {
        ovrTrackedController_Clear(&scene->SceneTransform.TrackedController[i]);
    }
...
ovrScene_Create(AAssetManager* amgr, XrInstance instance, XrSession session, ovrScene* scene) {
    // Simple ground plane and box geometry.
    {
        ...
        // AppSpaceWarp
        ovrProgram_Create(
            &scene->MotionVectorProgram,
            MOTION_VECTOR_VERTEX_SHADER,
            MOTION_VECTOR_FRAGMENT_SHADER);
    ...
    // Simple checkerboard pattern.
    {
        ...
        for (int y = 0; y < QUAD_HEIGHT; y++) {
            for (int x = 0; x < QUAD_WIDTH; x++) {
                ...
                int rgba = (0x88 << 24) | (gray << 16) | (g << 8) | r;
...
static void ovrScene_Destroy(ovrScene* scene) {
    ...
    // AppSpaceWarp
    ovrProgram_Destroy(&scene->MotionVectorProgram);
...
static void RenderScene(
    ...
    const ovrSceneMatrices* prevFrameSceneMatrices,
    const ovrSceneTransform* prevFrameSceneTransform,
    bool isMotionVectorPass) {
    // AppSpaceWarp
    const ovrProgram* selectProgram =
        isMotionVectorPass ? &scene->MotionVectorProgram : &scene->Program;

    for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
        ovrFramebuffer* frameBuffer = &renderer->FrameBuffer[eye];

        ovrFramebuffer_Acquire(frameBuffer, isMotionVectorPass);

        // Set the current framebuffer.
        ovrFramebuffer_SetCurrent(frameBuffer, isMotionVectorPass);
        GL(glUseProgram(selectProgram->Program));

        // Static background has same MODEL_MATRIX & PREV_MODEL_MATRIX
        XrMatrix4x4f modelMatrix;
        XrMatrix4x4f_CreateIdentity(&modelMatrix);
        glUniformMatrix4fv(
            selectProgram->UniformLocation[MODEL_MATRIX], 1, GL_FALSE, &modelMatrix.m[0]);

        if (selectProgram->UniformLocation[PREV_MODEL_MATRIX] >= 0) {
            glUniformMatrix4fv(
                selectProgram->UniformLocation[PREV_MODEL_MATRIX], 1, GL_FALSE, &modelMatrix.m[0]);
        }
        ...
        glUniformMatrix4fv(
            selectProgram->UniformLocation[VIEW_PROJ_MATRIX], 1, GL_FALSE, &viewProjMatrix.m[0]);

        if (selectProgram->UniformLocation[PREV_VIEW_PROJ_MATRIX] >= 0) {
            XrMatrix4x4f prevViewProjMatrix;
            XrMatrix4x4f_Multiply(
                &prevViewProjMatrix,
                &prevFrameSceneMatrices->ProjectionMatrix[eye],
                &prevFrameSceneMatrices->ViewMatrix[eye]);
            glUniformMatrix4fv(
                selectProgram->UniformLocation[PREV_VIEW_PROJ_MATRIX],
                1,
                GL_FALSE,
                &prevViewProjMatrix.m[0]);
        }
        ...
        if (isMotionVectorPass) {
            GL(glViewport(0, 0, frameBuffer->MotionVectorWidth, frameBuffer->MotionVectorHeight));
            GL(glScissor(0, 0, frameBuffer->MotionVectorWidth, frameBuffer->MotionVectorHeight));
        } else {
            GL(glViewport(0, 0, frameBuffer->Width, frameBuffer->Height));
            GL(glScissor(0, 0, frameBuffer->Width, frameBuffer->Height));
        }

        GL(glClearColor(0.02f, 0.02f, 0.1f, 1.0f));
        ...
        for (int i = 0; i < 4; i++) {
            if (scene->SceneTransform.TrackedController[i].Active == false) {
                continue;
            }
            ...
            glUniformMatrix4fv(
                selectProgram->UniformLocation[MODEL_MATRIX], 1, GL_FALSE, &model.m[0]);
            ...
            if (selectProgram->UniformLocation[PREV_MODEL_MATRIX] >= 0) {
                glUniformMatrix4fv(
                    selectProgram->UniformLocation[PREV_MODEL_MATRIX], 1, GL_FALSE, &model.m[0]);
            }
        ...
        // Render cubes
        for (int i = 0; i < ovrMaxCubeNumInstances; i++) {
            glUniformMatrix4fv(
                selectProgram->UniformLocation[MODEL_MATRIX],
                1,
                GL_FALSE,
                &scene->SceneTransform.CubeTransforms[i].m[0]);
            if (selectProgram->UniformLocation[PREV_MODEL_MATRIX] >= 0) {
                glUniformMatrix4fv(
                    selectProgram->UniformLocation[PREV_MODEL_MATRIX],
                    1,
                    GL_FALSE,
                    &prevFrameSceneTransform->CubeTransforms[i].m[0]);
            }
        ...
        ovrFramebuffer_Resolve(frameBuffer, isMotionVectorPass);
        ovrFramebuffer_Release(frameBuffer, isMotionVectorPass);
    }
}
...
static void ovrRenderer_RenderFrame(
    ...
    // Render main pass
    RenderScene(
        renderer, scene, sceneMatrices, prevFrameSceneMatrices, prevFrameSceneTransform, false);

    // AppSpaceWarp Render Motion vector pass
    RenderScene(
        renderer, scene, sceneMatrices, prevFrameSceneMatrices, prevFrameSceneTransform, true);
...
void UpdateStageBounds(ovrApp* pappState) {
    ...
    // using fixed ground plan for bound visual
    const float halfWidth = 1.5f;
    const float halfDepth = 1.5f;
...
static XrActionSet CreateActionSet(int priority, const char* name, const char* localizedName) {
    ...
    strncpy(asci.actionSetName, name, sizeof(asci.actionSetName));
    strncpy(asci.localizedActionSetName, localizedName, sizeof(asci.localizedActionSetName));
...
static XrAction CreateAction(
    ...
    strncpy(aci.actionName, actionName, sizeof(aci.actionName));
    strncpy(
        aci.localizedActionName,
        localizedName ? localizedName : actionName,
        sizeof(aci.localizedActionName));
...
void android_main(struct android_app* app) {
    ...
    const char* const requiredExtensionNames[] = {
        ...
        // AppSpaceWarp  Enable the extension
    ...
    strncpy(appInfo.applicationName, "OpenXR_SpaceWarp", sizeof(appInfo.applicationName));
    ...
    ALOGV(
        "SpaceWarp Properties: recommendedMotionVectorImageRectWidth=%d recommendedMotionVectorImageRectHeight=%d",
    ...
    while (app->destroyRequested == 0) {
        ...
        // Rotation around Y when moving thumbstick left <-> right
        if (appState.Focused) {
            rotationY -= appJoystickX * 0.04f;
        }

        const XrVector3f axisY = {0.0f, 1.0f, 0.0f};
        XrQuaternionf_CreateFromAxisAngle(&sceneMatrices.XrSpacePoseInWorld.orientation, &axisY, rotationY);
        ...
        XrQuaternionf_RotateVector3f(&forwardDir, &xfWorldFromHead.orientation, &forwardDir);

        if (appState.Focused) {
            XrVector3f scaled;
            XrVector3f_Scale(&scaled, &forwardDir, 0.1f * appJoystickY);
        ...
        for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
            ...
            XrPosef_Invert(&renderViewTransform, &xfWorldFromEye);
        ...
        for (int i = 0; i < 4; i++) {
            if (ActionPoseIsActive(controller[i], subactionPath[i])) {
                ...
                appState.Scene.SceneTransform.TrackedController[i].Active =
                    (lv.loc.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0;
                appState.Scene.SceneTransform.TrackedController[i].Pose = lv.loc.pose;
                for (int j = 0; j < 3; j++) {
                    float dt = 0.01f; // use 0.2f for for testing velocity vectors
                    (&appState.Scene.SceneTransform.TrackedController[i].Pose.position.x)[j] +=
                        (&lv.vel.linearVelocity.x)[j] * dt;
                }
            } else {
                ovrTrackedController_Clear(&appState.Scene.SceneTransform.TrackedController[i]);
            }
        }
...

        {
            ...
            if (moveJoystickState.changedSinceLastSync) {
                appJoystickX = moveJoystickState.currentState.x;
                appJoystickY = moveJoystickState.currentState.y;
            }
        }
        ...
        int enableDeterministicAnimationProp = -1;
        GetSystemPropertyInt("debug.oculus.enableDeterministicAnimation", &enableDeterministicAnimationProp);
        if (enableDeterministicAnimationProp >= 0) {
            EnableDeterministicAnimation = enableDeterministicAnimationProp > 0;
        }

        // Determine cube animation tic based on what animation mode is being used
        float cubeTic = 0.0f;
        if (EnableDeterministicAnimation) {
            cubeTic = CUBE_ANIMATION_SPEED * FRAME_BASED_CUBE_SPEED_FACTOR * frameIndex;
        } else {
            // Use Original GL logic
            cubeTic = CUBE_ANIMATION_SPEED * timeInSeconds;
        }

        // CubeAnimation
        {
            for (int z = 0; z < CUBE_COUNT_DIMENSION; z++) {
                for (int y = 0; y < CUBE_COUNT_DIMENSION; y++) {
                    for (int x = 0; x < CUBE_COUNT_DIMENSION; x++) {
                        const int instance =
                            (z * CUBE_COUNT_DIMENSION + y) * CUBE_COUNT_DIMENSION + x;

                        appState.Scene.SceneTransform.CubeTransforms[instance].m[0 * 4 + 0] = 1;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[0 * 4 + 1] = 0;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[0 * 4 + 2] = 0;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[0 * 4 + 3] = 0;

                        appState.Scene.SceneTransform.CubeTransforms[instance].m[1 * 4 + 0] = 0;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[1 * 4 + 1] = 1;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[1 * 4 + 2] = 0;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[1 * 4 + 3] = 0;

                        appState.Scene.SceneTransform.CubeTransforms[instance].m[2 * 4 + 0] = 0;
                        appState.Scene.SceneTransform.CubeTransforms[instance].m[2 * 4 + 1] = 0;
                        ...
                        const float cubePositionY = CUBE_ANIMATION_DISTANCE *
                            sin(cubeTic + y * CUBE_ANIMATION_Y_OFFSET +
                                z * CUBE_ANIMATION_Z_OFFSET + x * CUBE_ANIMATION_X_OFFSET);
        ...
        // AppSpaceWarp Cache previous frame transform for motion vector generation.
        static bool IsFirstFrame = true;
        static ovrSceneMatrices prevFrameSceneMatrices;
        static ovrSceneTransform prevFrameSceneTransform;

        if (IsFirstFrame) {
            prevFrameSceneMatrices = sceneMatrices;
            prevFrameSceneTransform = appState.Scene.SceneTransform;
        }
        ...
        if (shouldRenderWorldLayer) {
            ...
            for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
                ...
                // AppSpaceWarp : submit motion vector information XR runtime
                if (enableSpaceWarpForTheFrame) {
                    ...
                    proj_spacewarp_views[eye].motionVectorSubImage.swapchain =
                        frameBuffer->MotionVectorSwapChain.Handle;
                    ...
                    proj_spacewarp_views[eye].motionVectorSubImage.imageRect.extent.width =
                        frameBuffer->MotionVectorSwapChain.Width;
                    proj_spacewarp_views[eye].motionVectorSubImage.imageRect.extent.height =
                        frameBuffer->MotionVectorSwapChain.Height;
                    proj_spacewarp_views[eye].motionVectorSubImage.imageArrayIndex = 0;
                    proj_spacewarp_views[eye].depthSubImage.swapchain =
                        frameBuffer->MotionVectorDepthSwapChain.Handle;
                    ...
                    proj_spacewarp_views[eye].depthSubImage.imageRect.extent.width =
                        frameBuffer->MotionVectorDepthSwapChain.Width;
                    proj_spacewarp_views[eye].depthSubImage.imageRect.extent.height =
                        frameBuffer->MotionVectorDepthSwapChain.Height;
                    proj_spacewarp_views[eye].depthSubImage.imageArrayIndex = 0;
                    ...
                    XrPosef InvPrevFrameXrSpacePoseInWorld;
                    XrPosef_Invert(&InvPrevFrameXrSpacePoseInWorld, &PrevFrameXrSpacePoseInWorld);
                    ...
                    if (debugDeltaPose > 0) {
                        XrPosef_CreateIdentity(&proj_spacewarp_views[eye].appSpaceDeltaPose);
                        ALOGV("Bad appSpaceDeltaPose: watch carefully on background artifacts");
                    }
        ...
        if (ShowQuadLayer) {
            ...
            XrPosef InvXrSpacePoseInWorld;
            XrPosef_Invert(&InvXrSpacePoseInWorld, &sceneMatrices.XrSpacePoseInWorld);
            ...
            XrQuaternionf_CreateFromAxisAngle(&quad_layer_left.pose.orientation, &axis, 45.0f * MATH_PI / 180.0f);
            ...
            XrQuaternionf_CreateFromAxisAngle(&quad_layer_right.pose.orientation, &axis, 45.0f * MATH_PI / 180.0f);
