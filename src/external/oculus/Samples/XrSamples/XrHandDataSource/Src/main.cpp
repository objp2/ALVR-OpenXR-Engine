...
class XrHandDataSourceApp : public OVRFW::XrApp {
   private:
    static constexpr std::string_view kSampleExplanation =
        ...
        "                                                                                  \n"
        "The 'Set Hand Controller Type' / 'Set Hand Type to Natural' button toggles between\n"
        "a hand pose that is open, and one that is generated wrapped around the controller.\n"
        "                                                                                  \n"
        "The 'Render / Stop Rendering Tracked Remotes' button toggles displaying the       \n"
   ...
   public:
    ...
    virtual bool AppInit(const xrJava* context) override {
        ...
        bigText_ = ui_.AddLabel(
            "Open XR Hand Data Source Sample", {0.1f, 0.9f, -2.0f}, {1300.0f, 100.0f});
        bigText_->SetSurfaceColor(0 ,{0.0f, 0.0f, 1.0f, 1.0f});

        renderTrackedRemoteButton_ =
            ui_.AddButton("Render Tracked Remote", {-0.5f, -0.15f, -2.0f}, {500.0f, 100.0f}, [=]() {
                ...
                if (renderTrackedRemotes_) {
                    ...
                    renderTrackedRemoteButton_->SetText("Render Tracked Remotes");
                }
            });

        controllerHandDataTypeButton_ =
            ui_.AddButton("Set Hand Type Natural", {0.75f, -0.15f, -2.0f}, {500.0f, 100.0f}, [=]() {
        ...
        if (renderLeftController) {
            if (controllerRenderL_.IsInitialized()) {
                ...
                controllerRenderL_.Update(
                    in.LeftRemotePose, OVRFW::SimpleGlbRenderer::UpdateOffset_Grip);
            } else {
        ...
        if (renderRightController) {
            if (controllerRenderR_.IsInitialized()) {
                ...
                controllerRenderR_.Update(
                    in.RightRemotePose, OVRFW::SimpleGlbRenderer::UpdateOffset_Grip);
            } else {
