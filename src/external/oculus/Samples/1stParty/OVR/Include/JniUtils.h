...
// VrRuntimeClient was mainly developed for Android apps running Java, but also supports a subset of
// functionality with clients that do not have Java context. This macro is used to conditionally
// call into various parts of the runtime, and makes it easy to find where all these changes are
// made and then revisit them later on.
#define IF_JAVA_CONTEXT(_expr) (_expr)
#define IF_NO_JAVA_CONTEXT(_expr) (!(_expr))
...
// Get the code path of the current package.
inline const char*
ovr_GetFilesDir(JNIEnv* jni, jobject activityObject, char* filesDir, int const maxLen) {
    if (filesDir == NULL || maxLen < 1) {
        return filesDir;
    }
    ...
    JavaClass activityClass(jni, jni->GetObjectClass(activityObject));
    JavaClass fileClass(jni, jni->FindClass("java/io/File"));
    jmethodID getFilesDirId =
        jni->GetMethodID(activityClass.GetJClass(), "getFilesDir", "()Ljava/io/File;");
    jmethodID getAbsolutePathId =
        jni->GetMethodID(fileClass.GetJClass(), "getAbsolutePath", "()Ljava/lang/String;");
    if (getAbsolutePathId == 0 || getFilesDirId == 0) {
        OVR_LOG(
            "Failed to find getFilesDir or getAbsolutePath on class %llu, object %llu",
            (long long unsigned int)activityClass.GetJClass(),
            (long long unsigned int)activityObject);
        return filesDir;
    }

    JavaObject filesDirObject(jni, (jobject)jni->CallObjectMethod(activityObject, getFilesDirId));
    if (!jni->ExceptionOccurred()) {
        JavaUTFChars result(
            jni, (jstring)jni->CallObjectMethod(filesDirObject.GetJObject(), getAbsolutePathId));
        if (!jni->ExceptionOccurred()) {
            const char* path = result.ToStr();
            if (path != NULL) {
                OVR::OVR_sprintf(filesDir, maxLen, "%s", path);
            }
        } else {
            jni->ExceptionClear();
            OVR_LOG("Cleared JNI exception");
...
// Get the current process name, used for cases when there is no Java and no Activity or package
// pthread_getname_np() is only available on Android API >= 26
inline const char* ovr_GetCurrentProcessName(char* processName, int const maxLen) {
    processName[0] = '\0';

    int rc = pthread_getname_np(pthread_self(), processName, maxLen);
    if (rc != 0) {
        OVR_FAIL("ovr_GetCurrentProcessName pthread_getname_np() failed");
    }
    OVR_ASSERT(strlen(processName) > 0);
    OVR_LOG("ovr_GetCurrentProcessName non-Java process name '%s'", processName);
    return processName;
}
