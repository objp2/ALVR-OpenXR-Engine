...
static void UpdateFormat(
    ...
    if (format.Weight != format.LastWeight && format.Weight != 0xffffffff) {
        ...
        vertexParms[1] =
            (uint8_t)(clamp<float>(fontParms.ColorCenter + fontInfo.CenterOffset + w.ColorCenterOffset, 0.0f, 1.0f) * 255);
        vertexParms[0] =
            (uint8_t)(clamp<float>(fontParms.AlphaCenter + fontInfo.CenterOffset + w.AlphaCenterOffset, 0.0f, 1.0f) * 255);
    }
}
...
bool FontInfoType::LoadFromBuffer(void const* buffer, size_t const bufferSize) {
    ...
    if (jsonWeightArray.IsValid()) {
        ...
        for (; !jsonWeightArray.IsEndOfArray();) {
